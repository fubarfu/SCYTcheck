...
class TestUS1FullWorkflow:
    ...
    def mock_services(self) -> dict:
        ...
        ocr_service.extract_candidates.side_effect = (
            lambda lines, patterns=None, filter_non_matching=False: [(line, None) for line in lines]
        )
    ...
    def test_us1_complete_workflow(self, mock_services: dict, tmp_path: Path) -> None:
        ...
        frames = [region_selector.get_frame_at_time(url, t) for t in frame_times]
        ...
        csv_path = export_service.export_to_csv(analysis, str(tmp_path), filename)
    ...
    def test_scrollbar_time_navigation_workflow(self, mock_services: dict) -> None:
        ...
        expected_times = [(pos / 100.0) * duration for pos in scrollbar_positions]

        # Verify frame retrieval at each scrollbar position
        for _scrollbar_pos, expected_time in zip(
            scrollbar_positions, expected_times, strict=True
        ):
    ...
    def test_deduplicated_analysis_export_flow(self, tmp_path: Path) -> None:
        ...
        def iterate_frames_with_timestamps(
            ...
            yield from zip([1.0, 1.5, 4.2], frames, strict=True)
    ...
    def test_header_only_summary_export_when_no_names_detected(
        self, mock_services: dict, tmp_path: Path
    ) -> None:
        video_service = mock_services["video_service"]
        ocr_service = mock_services["ocr_service"]
...
def test_us3_regression_iterate_frames_contract_stays_compatible() -> None:
    ...
    def test_invalid_url_is_rejected_before_analysis_workflow(self, tmp_path: Path) -> None:
        ...
        def make_window(root_arg):
            window = _FakeWindow(
                root_arg, "https://youtube.com/watch?v=bad", str(tmp_path), settings
            )
    ...
    def test_region_selector_foreground_and_overlay_workflow(self, mock_services: dict) -> None:
        ...
        assert (
            selector._compute_overlay_y(frame_height=preview.shape[0], box_height=80)
            >= preview.shape[0] - 100
        )
