...
class FrameAssetStore:
    ...
    @staticmethod
    def frame_folder_for_csv(csv_path: Path | str) -> Path:
        file_path = Path(csv_path)
        return file_path.parent / f"{file_path.stem}_frames"

    def persisted_frame_path(self, csv_path: Path | str, candidate_id: str) -> Path:
        return self.frame_folder_for_csv(csv_path) / f"{candidate_id}.png"
