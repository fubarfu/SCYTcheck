...
- Q: How should extraction handle player names when names are expected to contain no blanks? → A: Enforce single-token extraction; after boundary extraction, keep one token only (nearest relevant boundary)
- Q: How should output `PlayerName` values be represented when deduplication still uses normalized keys? → A: Keep deduplication by normalized key, but output the earliest accepted on-screen extracted form (no normalization applied to displayed name)
...
- **FR-026**: When a context pattern matches, the app MUST extract the player name as follows: if only after-text is set, extract trimmed OCR text preceding the after-text match and keep only the last whitespace-delimited token; if only before-text is set, extract trimmed OCR text following the before-text match and keep only the first whitespace-delimited token; if both are set, extract trimmed text between the before-text match end and the after-text match start and keep only the first whitespace-delimited token. If no non-empty token remains, treat as no extracted name.
...
- **FR-028**: The app MUST deduplicate extracted player names across the entire analyzed video by normalized player name and output one row per normalized name. `StartTimestamp` in the output MUST represent the earliest merged appearance-event start for that player. The exported `PlayerName` for each normalized group MUST be the first accepted extracted on-screen form observed at the earliest timestamp for that normalized group.
